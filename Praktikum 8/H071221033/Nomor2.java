...
class TypeRacer {
    ...
    // Parameter Typer typer: objek peserta (Typer) yang akan ditambahkan hasil pengetikannya ke peringkat.
    ...
    public void startRace() {
        ...
        for (Typer typerss : raceContestant) {
            // memulai pengetikan oleh setiap peserta dengan memanggil method start(). Ini akan menjalankan metode run() dalam setiap objek Typer secara paralel dalam thread terpisah.
            typerss.start();
        }
        ...
        while (true) {
            ...
            if (isFinish) {
                ...
                // kondisi Jika jumlah peserta yang telah selesai mengetik != dengan jumlah total peserta  
                if (raceStanding.size() != raceContestant.size()) {
                    try {
                        // Memberikan jeda selama 1 detik untuk memperlambat pengecekan kemajuan penge
                        Thread.sleep(1000);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
...
public class Nomor2 {
    public static void main(String[] args) {
        ...
        typers[0] = new Typer("Bot Mansur", 68.0, typeRacer);
